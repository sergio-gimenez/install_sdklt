...
//Entry verification
while ((rv = bcmlt_entry_commit(efcp_hdl, BCMLT_OPCODE_TRAVERSE, BCMLT_PRIORITY_NORMAL)) == SHR_E_NONE) {
    if (bcmlt_entry_info_get(efcp_hdl, &e_info) != SHR_E_NONE || e_info.status != SHR_E_NONE) {
       break;
    }
 
    if (bcmlt_entry_field_get(efcp_hdl, s_ipc_dst_addr, &ipc_dst_addr) != SHR_E_NONE) {
       break;
    }
    
    if (bcmlt_entry_field_get(efcp_hdl, s_dst_port, &dst_port) != SHR_E_NONE) {
       break;
    }

    if (bcmlt_entry_field_get(efcp_hdl, s_mac_dst_addr, &mac_dst_addr) != SHR_E_NONE) {
       break;
    }

    printf("\n\nIPC destination address: %"PRId16"\n Port dst: %"PRId8"\n MAC dst: %"PRId64"\n", ipc_dst_addr, dst_port, mac_dst_addr);           
 }
